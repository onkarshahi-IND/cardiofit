...
func (fahe *FoodAlcoholHerbalEngine) filterClinicallySignificant(
	...
	for _, result := range results {
		...
		if result.Evidence == models.EvidenceLevelD {
			continue
		}
...
func (fahe *FoodAlcoholHerbalEngine) GetModifierInteractionCache(
	...
	if cached, found := fahe.cacheManager.GetModifierInteractions(cacheKey); found {
		// Type assert to ModifierInteractionResult slice
		// cached is []interface{}, convert each element to ModifierInteractionResult
		results := make([]ModifierInteractionResult, 0, len(cached))
		for _, item := range cached {
			if result, ok := item.(ModifierInteractionResult); ok {
				results = append(results, result)
			}
		}
		if len(results) > 0 {
